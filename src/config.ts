...
export const defaultConfig: OnlyneConfig = {
  ...
  outbound: {
    ...
    guardedExplicit: { reminders: 2, noOutputFallbackText: 'Onlyne/dsh error: no valid reply was produced.' },
    retry: { attempts: 2, concurrency: 8 },
  },
}

/** Same file pi-onlyne uses (`.pi/onlyne.json`), so pi and dsh share one config. */
export function configPath(cwd: string) {
  return join(cwd, '.pi', 'onlyne.json')
}
...
export function loadConfig(cwd: string): OnlyneConfig {
  ...
  return {
    ...
    ...parsed,
    watch: { ...defaultConfig.watch, ...parsed.watch },
    inbound: { ...defaultConfig.inbound, ...parsed.inbound, rules: parsed.inbound?.rules ?? [] },
    outbound: {
      ...defaultConfig.outbound,
      ...parsed.outbound,
      guardedExplicit: { ...defaultConfig.outbound.guardedExplicit, ...parsed.outbound?.guardedExplicit },
      retry: { ...defaultConfig.outbound.retry, ...parsed.outbound?.retry },
    },
  }
}

export function saveConfig(cwd: string, config: OnlyneConfig) {
  const path = configPath(cwd)
...
export function inboundModeFor(config: OnlyneConfig, channel: string, conversation?: string): InboundMode {
  return config.inbound.rules.find((r) => r.channel === channel && r.conversation === conversation)?.mode
    ?? config.inbound.rules.find((r) => r.channel === channel && !r.conversation)?.mode
    ?? config.inbound.defaultMode
}
